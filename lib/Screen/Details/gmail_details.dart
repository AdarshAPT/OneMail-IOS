...
import '../../Utils/custom_glow_behaviour.dart';
import '../../main.dart';

class GmailDetails extends StatefulWidget {
  const GmailDetails({Key? key, required this.email}) : super(key: key);
  final Gmail email;

  @override
  State<GmailDetails> createState() => _GmailDetailsState();
}

class _GmailDetailsState extends State<GmailDetails> {
            ...
            children: [
              ...
              Expanded(
                child: Wrap(
                  ...
                  children: bcc
                      .map<Widget>(
                        (Address address) => emailField(address.email, themes),
                      )
                      .toList(),
                ),
              ),
  ...
  Widget _showEmail() {
    return Obx(
      () => SizedBox(
        ...
        child: InAppWebView(
          initialData: InAppWebViewInitialData(data: widget.email.htmlPart!),
